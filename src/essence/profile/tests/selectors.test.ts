...
describe('Selectors Profile', () => {
  ...
  const mockedStateWithErrors: DeepPartial<StateSchema> = {
    profile: {
      validateError: [ValidateProfileError.INCORRECT_AGE, ValidateProfileError.INCORRECT_CURRENCY],
    },
  }
  ...
  test('Проверяем правильную работу getProfileValidateError', () => {
    expect(getProfileValidateError(mockedStateWithErrors as StateSchema)).toEqual([
