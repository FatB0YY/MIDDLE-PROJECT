...
export const getArticleRecommendations = recommendationsAdapter.getSelectors<StateSchema>(
  (state) => state.articleDetailaRecommendations || recommendationsAdapter.getInitialState()
)
...
const articleDetailsPageRecommendationsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchArticleDetailsRecommendationsListThunk.fulfilled, (state, action) => {
        state.isLoading = false
        recommendationsAdapter.setAll(state, action.payload)
      })
      .addCase(fetchArticleDetailsRecommendationsListThunk.rejected, (state, action) => {
        state.isLoading = false
